...
function Profile() {
    ...
    useEffect(() => {
        ...
        async function callApi() {
            try {
                let result = await fetch(`http://localhost:5000/v1/user/${auth._id}`)
                result = await result.json();
                setUserDetails(result);
                console.log("details", result)
                let result2 = await fetch(`http://localhost:5000/v1/userHistory/${auth._id}`)
    ...
    async function updateProfile(id, firstName, lastName, phone) {
        let result = await fetch(`http://localhost:5000/v1/update/${id}`, {
            method: 'PUT',
